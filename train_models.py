...
class CrossBatchMemory:
    ...
    def update(self, keys, labels):
        ...
        if ptr + batch_size <= self.queue_size:
            ...
            if ptr + batch_size == self.queue_size: self.is_full = True
        else:
...
def contrastive_loss(z, labels, memory_queue, margin=0.5):
    ...
    for i in range(N):
        ...
        if len(pos_sims) > 0: loss += torch.mean(1.0 - pos_sims)
        neg_sims = similarity_matrix[i][neg_mask]
        if len(neg_sims) > 0:
            neg_loss = neg_sims - margin
            active_neg_loss = neg_loss[neg_loss > 0]
            if len(active_neg_loss) > 0: loss += torch.mean(active_neg_loss)
    return loss / N
...
def run_single_experiment(exp_config, train_loader, val_loader, val_dataset, train_dataset_size, device, run_idx,
    ...
    # Bổ sung phần load mô hình không nằm trong timm, phải load từ local
    if "medvit" in model_str.lower():
        from models import MedViT
        if model_str == "medvit_small":
            model = MedViT.MedViT_small(num_classes=n_classes).to(device)
        elif model_str == "medvit_base":
            model = MedViT.MedViT_base(num_classes=n_classes).to(device)
        print(f"[+] Khởi tạo thành công Local Model: {model_str}")
    else:
    ...
    # Lấy Embedding Dimension an toàn
    with torch.no_grad():
    ...
    model.eval()
    if is_ce and hasattr(model, 'reset_classifier'): model.reset_classifier(0)
...
def main():
    ...
    print(f"[+] Tổng số ảnh tập Huấn luyện: {train_size} ảnh")
    ...
    for config in EXPERIMENTS:
        ...
        # -----------------------------------------------------------------
        # ĐIỀU CHỈNH BATCH SIZE THEO MÔ HÌNH ĐỂ TRÁNH TRÀN VRAM GPU (4GB TRÊN GTX 3050 Ti)
        # -----------------------------------------------------------------
        if "medvit" in model_str.lower():
        ...
        print(f"\n" + "=" * 80)
        print(f"[*] BẮT ĐẦU HUẤN LUYỆN: {model_name} | LOSS: {loss_type} | BATCH SIZE: {current_batch_size}")
        print("=" * 80)

        # KHỞI TẠO DATALOADER BÊN TRONG VÒNG LẶP
        ...
        # KẾT THÚC ĐẾM THỜI GIAN & TÍNH TOÁN
        end_time = time.time()
        ...
        # Tính độ lệch chuẩn (std) duy nhất cho cột mAP theo đúng bài báo
        std_map = np.std(run_metrics['mAP']) if NUM_RUNS > 1 else 0.00

        print(f"\n[+] KẾT QUẢ TỔNG HỢP {model_name} (Over {NUM_RUNS} runs):")
        print(f"    Thời gian hoàn thành: {formatted_time}")  # <--- In thời gian chạy
    ...
    # =====================================================================
    # KHỐI LỆNH IN BẢNG TABLE 1 HOÀN CHỈNH Ở CUỐI FILE
    # =====================================================================
    print("\n\n" + "=" * 115)
    print(f"{'Table 1: Medical Image retrieval results':^115}")
    print("-" * 115)
    print(
